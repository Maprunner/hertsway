...
const createProfile = (leg) => {
  ...
  try {
    ...
    const { height, distance } = processFile(olddata)
    ...
    // create profile png
    bookHeightProfile(data).then((svg) => {
...
const leg = 1
let doResize = false
const outDir = site.imageBase + 'images/book/'

for (let leg = 1; leg < 31; leg = leg + 1) {
  createProfile(leg)
  const legName = 'leg' + leg
