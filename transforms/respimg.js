 ...
 */
// Modified version of eleventy-plugin-local-respimg plugin
// for blog imge set-up. Use this file to overwrite the node_modules
//  version until I work out how to do this properly
...
function respimgSetup(userConfig = {}) {
  ...
  return async function respimg(content, outputPath) {
    ...
    ;(
      await glob(
        config.images.additional.map((i) => path.join(config.folders.source, i))
      )
    ).forEach((pth) => {
      const f = {
        src: pth,
    ...
    if (outputPath && outputPath.endsWith('.html')) {
      ...
      if (images.length) {
        for (let i = 0; i < images.length; i++) {
          ...
          const { src, local } = determineImagePath(
            $(image).attr('src'),
            outputPath,
            config.folders.output
          )

          if (local && src.slice(-3) !== 'png') {
            ...
            if (config.images.gifToVideo && type.mime === 'image/gif') {
              ...
              const video = `<video src="${generateSrcset(
                ['xform'],
                src,
                'mp4'
              ).slice(
                0,
                -7
              )}" type="video/mp4" autoplay loop muted playsinline controls height="${
                size.height
              }" width="${size.width}">${$(image).attr('alt')}</video>`
              ...
              if (optimize) {
                await Promise.all(
                  (
                    await Promise.all(
                      await resizeAndOptimize(sizes, type, src, file, config)
                    )
                  )
                    .flat()
                    .map((f) => writeFile(f.dest, f.buff))
                )
              }
